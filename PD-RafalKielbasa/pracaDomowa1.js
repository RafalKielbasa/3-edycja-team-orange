...
const reverse = (string, firtsParam, secondParam) =>
  string
    .slice(firtsParam, secondParam)
    .split('')
    .reverse()
    .join('')
    .toLowerCase()

const peopleWithNicknames = people.map((item) => {
  const { firstName, lastName } = item
  const firstPart = reverse(firstName, 0, 3)
  const nickname =
    firstPart[0].toUpperCase() + firstPart.slice(1) + reverse(lastName, -3)
...
function introduceYourself() {
  ...
  console.log(
    `Cześć jestem ${firstName} ${lastName}, ale w szkole mówią na mnie [${nickname}]`
  )
}

const introducePeple = peopleWithNicknames.map((item) => ({
  ...item,
  introduceYourself,
}))

// introducePeple.forEach((item) => item.introduceYourself())
...
function checkFunction(num, object) {
  let keysLenght = 0
  ...
  for (key in object) {
    keysLenght += object[key].length
  }
  const whitchColor = Math.abs(keysLenght - num) % colorsLength
  console.log(colors[whitchColor])
}

function getFavouriteColor(number = 5) {
  checkFunction(number, this)
}

const introducePepleAndColors = introducePeple.map((item) => ({
  ...item,
  getFavouriteColor,
}))

// introducePepleAndColors[2].getFavouriteColor()
...
const notInObject = (object, number = 5) => {
  checkFunction(number, object)
}

notInObject(introducePeple[0])

for (object of introducePeple) {
  notInObject(object)
}
